...
LOKI_VERSION="${LOKI_VERSION:-6.10.0}" 
...
if [[ "$SKIP_LOKI" != "true" ]]; then
  ...
  wait_for_rollout "daemonset/loki-canary" "$OTEL_NAMESPACE"
  wait_for_rollout "statefulset/loki-chunks-cache" "$OTEL_NAMESPACE" "180s"
else
